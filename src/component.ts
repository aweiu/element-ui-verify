...
const ElFormItemMethods: { [methodName: string]: Function } = (FormItem as any)
  .methods;
@Component
export default class ElFormItemVerifyComponent extends Vue {
  ...
  onValidateMessageChanged(msg: string) {
    if (this._verify && msg !== "") {
      ...
      (this as any).validateMessage = errorMessage.macroToValue(
    ...
    if (fieldValue === "") {
      ...
      for (let name in ruleGetters) {
        ...
        if (ruleVal !== undefined)
          asyncVerifyRules = asyncVerifyRules.concat(
            ruleGetters[name](ruleVal)
          );
      }
      // 统一处理错误提示（代码块放在此处可以只针对普通规则）
      if (this.errorMessage !== undefined) {
        for (let rule of asyncVerifyRules)
          (rule as any).message = this.errorMessage;
      }
      // 自定义校验方法置后
  ...
  onFieldChange() {
    const fieldChange =
      this.fieldChange || ElFormItemVerifyComponent.fieldChange;
